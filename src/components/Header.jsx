  ...
  return <>
    <header className="topbar">
      ...
      <div className="top-actions">
        ...
        <button className="icon-button" onClick={showNotifications} aria-label="Notifications"><Icon name="bell" size={18}/>{unread > 0 && <i/>}</button>
        <button className="theme-switch" onClick={onToggle} aria-pressed={dark}><span className="theme-thumb">{dark ? 'M' : 'S'}</span></button>
        <span className="avatar avatar-lia">HR</span>
      </div>
    </header>
    {open && <div className="header-popover notification-popover">
      <div><h2>Notifications</h2><button onClick={() => setOpen(false)}>×</button></div>
      {notifications.length ? notifications.map(item => <article key={item.id}><b>{item.title}</b><p>{item.message}</p><small>{new Date(item.created_at).toLocaleString()}</small></article>) : <p>No workflow notifications yet.</p>}
    </div>}
  </>
}
