...
router.delete('/templates/:id', authorize('hr'), async (req, res, next) => { try { const { rows } = await query('UPDATE certificate_templates SET is_active=false,updated_at=NOW() WHERE id=$1 AND is_active=true RETURNING *', [req.params.id]); if (!rows[0]) return res.status(404).json({ error: 'Active certificate template not found.' }); try { await logActivity({ req, user: req.user, action: 'certificate.template_retire', category: 'certificate', targetId: req.params.id, description: `${req.user.name || req.user.email || 'HR'} retired certificate template ${rows[0].name}` }) } catch (_) {} res.json({ retired: true }) } catch (error) { next(error) } })
